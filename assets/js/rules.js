/* ============================================================
   rules.js — HOA Rules & Regulations module
   Mirrors announcements.js's create/edit/delete/upload pattern.
   Stored at HOARules > {ruleId} > { title, category, description,
   link, imageUrl, postedByName, postedByRole, dateCreated,
...
renderShell("rules", adminProfile, { breadcrumb: "Rules & Regulations" });
...
/** Max characters allowed in a rule's description. */
...
content.innerHTML = `
  <div class="page-header">
    <div>
      <div class="page-header__title">Rules & Regulations</div>
      <div class="page-header__subtitle">Publish community rules and policies for residents.</div>
    </div>
    <div class="page-header__actions">
      <button class="btn btn-primary" id="newRuleBtn">
  ...
  <div class="stat-grid" id="ruleStats"></div>
  <div class="card"><div id="ruleTableRoot"></div></div>
`;
...
const table = new DataTable({
  root: document.getElementById("ruleTableRoot"),
  ...
  emptyTitle: "No rules posted yet",
  emptyDesc: "Publish your first rule or regulation to get started.",
  onRowClick: (row) => openPreviewModal(row),
});

onValue(ref(db, DB_PATHS.hoaRules), (snap) => {
  allRules = objectToArray(snap.val(), "id").sort(
    (a, b) => Number(b.timestamp || 0) - Number(a.timestamp || 0),
  );
  table.setData(allRules);
  renderStats();
});
...
function renderStats() {
  ...
  const thisMonth = allRules.filter((r) => {
...
table.cfg.afterRender = (rows) => {
  ...
  document
    ...
    .forEach((btn, i) =>
      btn.addEventListener("click", () => deleteRule(rows[i])),
    );
};
...
function openPreviewModal(r) {
  const overlay = openModal({
    ...
    subtitle: `${r.category || "General"} · Posted by ${r.postedByName || "Admin"}`,
    size: "modal-lg",
    bodyHTML: `
      <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;">
        <span class="badge badge-neutral">${escapeHtml(r.category || "General")}</span>
      </div>
      ${r.imageUrl ? `<div style="margin-bottom:16px;"><img src="${escapeHtml(r.imageUrl)}" alt="Rule image" style="max-width:100%;border-radius:10px;display:block;"></div>` : ""}
...
function openEditorModal(existing) {
  ...
  const overlay = openModal({
    ...
    subtitle: isEdit
      ...
      : "Publish a rule or regulation visible to all residents",
    size: "modal-lg",
    bodyHTML: `
      <div class="field" id="titleField">
        <label>Title</label>
        <input class="input" id="titleInput" placeholder="e.g. Guest Vehicle Parking Policy" value="${escapeHtml(existing?.title || "")}">
      ...
      <div class="field" id="descField">
        ...
        <textarea class="textarea" id="descInput" style="min-height:180px;" maxlength="${DESCRIPTION_MAX_LENGTH}" placeholder="Write the full rule or policy…">${escapeHtml(existing?.description || "")}</textarea>
    ...
    footerHTML: `
      ...
      <button class="btn btn-primary" data-act="save">${isEdit ? "Save Changes" : "Publish Rule"}</button>
    `,
  });
  ...
  overlay
    ...
    .addEventListener("click", async () => {
      ...
      try {
        if (isEdit) {
          // Editing never touches ruleId/postedBy*/dateCreated/timeCreated/timestamp —
          ...
          const newRef = push(ref(db, DB_PATHS.hoaRules));
          const fullPayload = {
            ...payload,
            ruleId: newRef.key,
...
async function deleteRule(r) {
  const ok = await confirmDialog({
    title: "Delete rule",
    message: `Delete "<strong>${escapeHtml(r.title || "this rule")}</strong>"? This cannot be undone.`,
    tone: "danger",
    confirmLabel: "Delete",
  });
  if (!ok) return;
...
/* ============================================================
   PRINT — All rules within a chosen date range.
   ============================================================ */

function openPrintRangeModal() {
  const overlay = openModal({
    title: "Print Rules & Regulations",
    subtitle: "Generate a printable list of rules for a date range",
    bodyHTML: `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
      ...
      <p style="font-size:12px;color:var(--color-grey);margin-top:12px;line-height:1.6;">
        Every rule posted inside this range will be included.
      </p>
    `,
    footerHTML: `
      <button class="btn btn-secondary" data-act="cancel">Cancel</button>
  ...
  overlay.querySelector('[data-act="print"]').addEventListener("click", () => {
    ...
    printRulesRange(fromVal, toVal);
    overlay.close();
  });
}

function printRulesRange(fromVal, toVal) {
  ...
  if (!rows.length) {
    toast({
      ...
      desc: "No rules fall inside that date range.",
    });
    return;
  }
  ...
  printHTML(
    `Rules & Regulations — ${formatDate(from)} to ${formatDate(to)}`,
    bodyHTML,
  );
  toast({
    type: "success",
    title: "Print ready",
    desc: `${rows.length} rule${rows.length === 1 ? "" : "s"} included.`,
  });
}
